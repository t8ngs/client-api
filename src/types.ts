...
// Node 18+ provides a global Blob, but its structural type may differ from import('buffer').Blob
// Superagent type definitions (and downstream .field/.attach) often reference the buffer module Blob.
// To avoid incompatibilities ("property 'slice' is missing" errors), we explicitly re-export the
// buffer Blob type alias and use it in MultipartValue so TypeScript performs structural compatibility
// with the expected union.
 ...
 */
// Use Node's Blob type explicitly to align with superagent and Node runtime.
export type MultipartValue = NodeBlob | Buffer | ReadStream | string | boolean | number
