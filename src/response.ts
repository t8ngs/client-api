...
export class ApiResponse extends Macroable {
   ...
   */
    ...
    if (!this.assert) {
      throw new Error(
        'Response assertions are not available. Make sure to install the @t8ngs/assert plugin'
      )
    }
  }
   ...
   */
    ...
    if ('isValidApiResponse' in this.assert! === false) {
      throw new Error(
        'OpenAPI assertions are not available. Make sure to install the @t8ngs/openapi plugin'
      )
    }
  }
